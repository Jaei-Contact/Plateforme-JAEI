...
export default function ArticlesPage() {
  ...
  // domainFilter — free-text domain filter
  ...
  const buildParams = useCallback((q, domain, p) => {
  ...
  const applySearch = (e) => {
    ...
    setSearch(q);
    setSearchParams(buildParams(q, domainFilter, 1));
  };

  const applyDomainFilter = (e) => {
    e.preventDefault();
    const d = domainInput.trim();
    setDomainFilter(d);
    setSearchParams(buildParams(search, d, 1));
  };

  const clearDomainFilter = () => {
    setDomainFilter('');
    setDomainInput('');
    setSearchParams(buildParams(search, '', 1));
  };

  const goToPage = (p) => {
    setSearchParams(buildParams(search, domainFilter, p));
  ...
  const hasFilters = search || domainFilter;
          ...
          <div className="flex gap-6 items-start">
            ...
            <aside className="hidden lg:block w-60 flex-shrink-0 self-start sticky top-20">
              <div className="bg-white border border-neutral-200 rounded overflow-hidden">
                ...
                {/* Free-text domain filter */}
                <div className="px-4 py-3">
                  <form onSubmit={applyDomainFilter} className="flex flex-col gap-2">
                    <input
                      type="text"
                      value={domainInput}
                      onChange={e => setDomainInput(e.target.value)}
                      placeholder="e.g. Agroecology…"
                      className="w-full px-3 py-2 text-xs border border-neutral-300 rounded
                                 focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary
                                 bg-white text-neutral-800 placeholder:text-neutral-400"
                    />
                    <button type="submit"
                            className="w-full py-1.5 text-xs font-semibold text-white bg-primary
                                       rounded hover:bg-primary-600 transition-colors">
                      Apply
                    </button>
                  </form>
                  {domainFilter && (
                    <button onClick={clearDomainFilter}
                            className="mt-2 w-full text-xs text-error border border-error/30 rounded
              ...
              <div className="bg-white border border-neutral-200 rounded px-4 py-3 mb-4
                ...
                {hasFilters && (
                  <div className="flex flex-wrap gap-2">
                    ...
                    {domainFilter && (
                      <span className="inline-flex items-center gap-1 text-xs bg-accent-50
                ...
                {/* Filtre domaine mobile */}
                <form onSubmit={applyDomainFilter}
                      className="flex gap-2 lg:hidden w-full pt-2 border-t border-neutral-100">
                  <input
                    type="text"
                    value={domainInput}
                    onChange={e => setDomainInput(e.target.value)}
                    placeholder="Filter by domain…"
                    className="flex-1 px-3 py-1.5 text-xs border border-neutral-300 rounded
                               focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary
                               bg-white text-neutral-800 placeholder:text-neutral-400"
                  />
                  <button type="submit"
                          className="px-3 py-1.5 text-xs font-semibold text-white bg-primary
                                     rounded hover:bg-primary-600 transition-colors flex-shrink-0">
                    Filter
                  </button>
                  {domainFilter && (
                    <button type="button" onClick={clearDomainFilter}
                            className="px-2 py-1.5 text-xs text-error border border-error/30 rounded
